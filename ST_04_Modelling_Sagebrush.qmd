...
serv_conn <- "D:/AuroraUAV/Cheatgrass"
...
fd_p <- file.path(serv, "Modelling/Field Data", proj, paste0(
  proj, "sagebrush_modelling_data.gpkg"
))
sage_pres <- st_read(fd_p, "presence", quiet = TRUE) %>%
  filter(!is.na(presence)) %>%
  mutate(presence = factor(presence, levels = c("TRUE", "FALSE")))
sage_cover <- st_read(fd_p, "cover", quiet = TRUE)
cc_ids <- c(
  "Free (0%)", "Trace (1-5%)", "Light Infestation (5-25%)",
  "Mild Infestation (25-50%)", "Sagebrush Dominated (50-100%)"
)
sage_cc <- sage_cover %>%
...
cov_layrs <- cov_layrs[grep(
  paste(names(sage_pres), collapse = ".tif|"),
  basename(cov_layrs)
)]
covs <- rast(cov_layrs)
...
bin_msrs <- mlr3measures::confusion_matrix(ml_prediction$truth, ml_prediction$response, positive = "TRUE")
...
prob_path <- file.path(out_dir, paste0(proj, "_sagebrush_probability_model.tif"))
...
prob_predict <- terra::predict(
  ...
  filename = prob_path, overwrite = TRUE, wopt = list(
    steps = length(bs[[1]]),
    names = paste0(proj, "_sagebrush_probability_model")
  )
)
...
prob_classes <- writeRaster(prob_classes, file.path(
  out_dir, paste0(proj, "_sagebrush_probability_model_classified.tif")
),
...
saveRDS(ranger_model, file.path(
  out_dir, paste0(proj, "_sagebrush_probability_model.rds")
))
write.csv(cbind(tsk_pres$data(), tsk_pres$coordinates()), file.path(
  out_dir, paste0(proj, "_sagebrush_probability_model_data.csv")
),
row.names = FALSE
)
write.ftable(ftable(ml_confusion), file.path(
  out_dir, paste0(proj, "_sagebrush_probability_confusion_matrix.csv")
),
quote = FALSE, sep = ","
)
...
tsk_cover <- as_task_regr_st(sage_cover, target = "cover")
...
cover_path <- file.path(out_dir, paste0(proj, "_sagebrush_cover.tif"))
...
cover_predict <- terra::predict(
  ...
  filename = cover_path, overwrite = TRUE, wopt = list(
    ...
    names = paste0(proj, "_sagebrush_cover_model")
  )
)
...
cover_classes <- writeRaster(cover_classes, file.path(
  out_dir, paste0(proj, "_sagebrush_cover_model_classified.tif")
),
overwrite = TRUE, datatype = "INT1U"
)
...
saveRDS(cover_ranger_model, file.path(
  out_dir, paste0(proj, "_sagebrush_cover_model.rds")
))
write.csv(cbind(tsk_cover$data(), tsk_cover$coordinates()), file.path(
  out_dir, paste0(proj, "_sagebrush_cover_model_data.csv")
),
row.names = FALSE
)
...
# Extract model and set parameters
cc_path <- file.path(out_dir, paste0(proj, "_sagebrush_cover_class_model.tif"))
...
cc_predict <- terra::predict(
  ...
  overwrite = TRUE, wopt = list(
    ...
    names = paste0(proj, "_sagebrush_cover_class_model"),
    datatype = "INT1U"
  )
)
...
cls <- data.frame(
  ...
  class = c(
    "Free (0%)", "Trace (1-5%)", "Light Infestation (5-25%)",
    "Mild Infestation (25-50%)", "Bheatgrass Dominated (50-100%)"
  ),
  col = c("green", "deepskyblue", "yellow", "orange", "red")
)
...
saveRDS(cc_ranger_model, file.path(
  out_dir, paste0(proj, "_sagebrush_cover_class_model.rds")
))
write.ftable(ftable(cc_ml_confusion), file.path(
  out_dir, paste0(proj, "_sagebrush_cover_class_confusion_matrix.csv")
),
quote = FALSE, sep = ","
)
...
ggsave(
  file.path(out_dir, paste0(
    proj, "_sagebrush_probability_model_importance.png"
  )),
  plot = prob_varimp_fig, width = 6.5, height = 6.5, units = "in"
)
ggsave(
  file.path(out_dir, paste0(
    proj, "_sagebrush_cover_model_importance.png"
  )),
  plot = cover_varimp_fig, width = 6.5, height = 6.5, units = "in"
)
ggsave(
  file.path(out_dir, paste0(
    proj, "_sagebrush_cover_class_model_importance.png"
  )),
