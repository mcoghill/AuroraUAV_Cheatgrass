...
mem <- as.numeric(system("awk '/MemTotal/ {print $2}' /proc/meminfo", intern = TRUE))
terra_orig <- terraOptions()
terraOptions(memfrac = 0.8, memmin = round((mem / 1024 / 1024) * (3/4)))

# Start SAGA GIS 9.5.0
saga_path <- "C:/SAGA-GIS/saga-9.5.0_x64/saga_cmd.exe"
...
kmz_files <- list.files(
  proc_dir, pattern = ".kmz$", full.names = TRUE, recursive = TRUE)
...
# Unzip KMZ to KML
flights <- lapply(kmz_files, function(x) {
...
# Create tile and DTM directories
tile_dir <- file.path(layr_dir, "01_LAS")
dtm_dir <- file.path(layr_dir, "02_DTM")
invisible(sapply(c(tile_dir, dtm_dir), dir.create, showWarnings = FALSE))

set_lidr_threads(0)
las_proc <- lapply(rgb_las_files, function(x) {
  
  # Set some file parameters/input/output paths
  kml_file <- list.files(dirname(x), pattern = ".kml$", full.names = TRUE)
  tile_out <- file.path(tile_dir, strsplit(basename(dirname(x)), "_")[[1]][1])
  dir.create(tile_out, showWarnings = FALSE)
  dtm_raw_out <- file.path(
    dtm_dir, paste0(strsplit(basename(dirname(x)), "_")[[1]][1], "_DTM_raw.tif"))
  ...
  ctg <- readLAScatalog(x, chunk_size = 250, chunk_buffer = 0, progress = FALSE)
  ...
  tin_pipe <- triangulate(filter = "-keep_class 2 9 -drop_class 18", ofile = "")
  dtm_pipe <- lasR::rasterize(res = 0.5, tin_pipe, ofile = dtm_raw_out)
  norm_pipe <- transform_with(tin_pipe)
  chm_pipe <- lasR::rasterize(res = 0.5, operators = "max", ofile = "")
  ...
  names(dtm_chm_raw) <- paste0(
    strsplit(basename(dirname(x)), "_")[[1]][1], c("_DTM", "_CHM"))
...
# Create output directory, define input file lists
cov_dir <- file.path(layr_dir, "04_Covariates")
dir.create(cov_dir, showWarnings = FALSE)
dtm_chm_list <- do.call(c, lapply(las_proc, sources))

dtm_chm_mos <- lapply(c("DTM", "CHM"), function(x) {
  ...
  rast_mosaic <- saga$grid_tools$mosaicking(
    target_out_grid = file.path(tempdir(), "DTM.tif"),
    file_list = file.path(tempdir(), "dtm_file_list.txt"),
    overlap = 6,
    blend_dist = 10,
    blend_bnd = 1,
    match = 3)
  
  # Fix the CRS information
  terra::crs(rast_mosaic) <- terra::crs(las_proc[[1]])
  rast_mosaic <- writeRaster(rast_mosaic, file.path(
    cov_dir, paste0(x, ".tif")), overwrite = TRUE)
})

dtm_mosaic <- dtm_chm_mos[[1]]
chm_mosaic <- dtm_chm_mos[[2]]
...
raw_dtms <- list.files(dtm_dir, pattern = "_DTM.tif$", full.names = TRUE)

# Create an object of periods to collect data from. Not all periods from 
# ClimateBC/NA are included, but many common ones are. If you need ones not
# included in this package, you will have to consult the other script files
# generated to see the usage of the ClimateNA_cmdLine() function.
pl <- c("Year_2023.ann")

# Create an object of variables to generate. This can be a single variable
# (e.g.: mean annual temperature (MAT)), multiple variables (e.g.: c("MAT", 
# "MAP")), or letters representing all annual (Y), seasonal (S), or monthly (M)
# variables. These letters can be combined in a single string to generate all 
# variables as well, e.g.: "YSM".
vl <- paste0(c(
...
int_ids <- paste0(c("PPT", "DD_0", "DD5", "DD_18", "DD18", "DD1040", "NFFD", 
                    "PAS", "Eref", "CMD", "RH"), "_sm")

# Create a climate variable folder for storing each output by flight area
clm_dir <- file.path(layr_dir, "03_Climate")
dir.create(clm_dir, showWarnings = FALSE)
...
clm_par <- future_lapply(raw_dtms, function(x) {
  clm_subdir <- file.path(clm_dir, gsub(".tif", "", basename(x)))
  ...
  dtm_wgs <- dtm_bca %>% 
    project("EPSG:4326", threads = FALSE, filename = file.path(
      tempdir(), basename(x)), overwrite = TRUE)
  ...
  # time, but will generate .tif files for each variable it outputs. This function
  # only outputs the SpatRaster object from the final entry in the periodList 
  # argument, so you will need to reload all rasters.
  clm <- climateNAr(dtm_in, periodList = pl, varList = vl, outDir = clm_dest)
  rm(clm)
  gc()
  
  clm_files <- list.files(paste0(clm_dest, gsub(".tif", "", basename(dtm_in))),
                          recursive = TRUE, pattern = ".tif$", full.names = TRUE)
  clm <- rast(clm_files)
  
  # Reprojection: don't save outputs here, keep these in memory!
  ...
  clm_int_bca <- project(clm_int, dtm_bca, threads = FALSE, method = "near")
  ...
  clm_num_bca <- project(clm_num, dtm_bca, threads = FALSE, method = "bilinear")
...
clm_mosaics <- do.call(c, lapply(names(clm_par[[1]]), function(x) {
  ...
  clm_mosaic <- saga$grid_tools$mosaicking(
    target_out_grid = file.path(tempdir(), paste0(x, ".tif")),
  ...
  # Assign CRS information, and write file to covariate directory
  terra::crs(clm_mosaic) <- terra::crs(clm_var[[1]])
  clm_mosaic <- writeRaster(
    clm_mosaic, file.path(cov_dir, paste0(x, ".tif")), overwrite = TRUE)
}))
...
refl_out <- file.path(layr_dir, "05_Reflectance")
dir.create(refl_out, showWarnings = FALSE)
ms_lowres <- lapply(refl_dirs, function(x) {
  
  # Define inputs and outputs
  flight_id <- strsplit(basename(dirname(x)), "_")[[1]][1]
  kml_file <- list.files(dirname(x), pattern = ".kml$", full.names = TRUE)
  r_files <- list.files(x, pattern = ".tif$", full.names = TRUE)
  ...
  r_out_dir <- file.path(refl_out, flight_id)
  r_out <- file.path(r_out_dir, paste0(r_names, ".tif"))
  dir.create(r_out_dir, showWarnings = FALSE)
  kml <- suppressWarnings(vect(kml_file, what = "geoms")) %>% 
    project(terra::crs(r))
  dtm_file <- file.path(layr_dir, "02_DTM", paste0(flight_id, "_DTM.tif"))
  dtm <- rast(dtm_file)
  ...
  rr <- resample(r, dtm, method = "cubicspline", threads = TRUE) %>% 
    mask(kml) %>% 
...
bands_mos <- do.call(c, lapply(names(ms_lowres[[1]]), function(x) {
  ...
  band_out <- file.path(refl_out, "Bands")
  ...
  # Fix incorrect CRS information
  terra::crs(band_mosaic) <- terra::crs(band[[1]])
  band_mosaic <- writeRaster(
    band_mosaic, file.path(band_out, paste0(x, ".tif")), overwrite = TRUE)
}))
...
s <- spectralIndices(
  ...
  redEdge2 = "red edge") %>% 
  writeRaster(file.path(cov_dir, paste0("MS_", names(.), ".tif")),
              overwrite = TRUE)
...
dtm <- rast(file.path(cov_dir, "DTM.tif"))
od <- normalizePath(file.path(cov_dir, "Terrain_"), mustWork = FALSE)
...
covs <- c(dtm_mosaic, chm_mosaic, clm_mosaics, s, ta_basic, ta_custom)
