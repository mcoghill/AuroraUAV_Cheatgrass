...
ls <- c("tidyverse", "terra", "sf", "lidR", "lasR", "Rsagacmd", "ClimateNAr",
        "RStoolbox", "future.apply")
invisible(
  suppressPackageStartupMessages(lapply(ls, library, character.only = TRUE)))
rm(ls)
...
ms_dir <- file.path(layr_dir, "04_Multispectral")
cov_dir <- file.path(layr_dir, "05_Covariates")
if(dir.exists(layr_dir) && sanitize) unlink(layr_dir, recursive = TRUE)
dir.create(layr_dir, showWarnings = FALSE)
invisible(
  sapply(c(tile_dir, dtm_dir, clm_dir, cov_dir, ms_dir),
...
las_proc <- lapply(rgb_las_files, function(x) {
  ...
  chm_pipe <- lasR::rasterize(
    res = 0.5, operators = "max", filter = "-drop_z_below 0", ofile = "")
...
dtm_chm_mos <- lapply(c("DTM", "CHM"), function(x) {
  ...
  unlink(file.path(tempdir(), paste0(x, ".tif")))
  return(rast_mosaic)
})
...
dtm_list <- list.files(dtm_dir, pattern = "_DTM.tif$", full.names = TRUE)
pl <- c("Year_2023.ann")
vl <- paste0(c(
...
clm_par <- future_lapply(dtm_list, function(x) {
  clm_subdir <- file.path(clm_dir, gsub(".tif", "", basename(x)))
  dir.create(clm_subdir, showWarnings = FALSE)
  
  flight_id <- gsub("_DTM", "", basename(clm_subdir))
  kml_file <- gsub(
    ".kmz$", ".kml", 
    kmz_files[basename(kmz_files) == paste0(flight_id, ".kmz")][1])
...
ms_lowres <- lapply(refl_dirs, function(x) {
  ...
  kml <- suppressWarnings(vect(kml_file, what = "geoms")) %>% 
    project(terra::crs(r))
  dtm_file <- file.path(dtm_dir, paste0(flight_id, "_DTM.tif"))
  dtm <- rast(dtm_file)
...
# Move files
cov_files <- list.files(cov_dir, full.names = TRUE, pattern = ".tif$")
file.rename(cov_files, file.path(layr_dir, basename(cov_files)))
rm(cov_files)

# Cleanup
saga_remove_tmpfiles()
unlink(cov_dir, recursive = TRUE)
rm(cov_dir)

covs <- rast(list.files(layr_dir, pattern = ".tif", full.names = TRUE))
