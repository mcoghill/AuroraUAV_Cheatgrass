...
ls <- c("tidyverse", "readxl", "terra", "sf", "corrplot", "caret")
...
kml_dir <- file.path(
  serv,
  "KML Flight Areas/South Thompson/South Thompson Project Area/overall area"
)
fd_dir <- file.path(serv, "Vegetation Surveys", proj)
air_photo_p <- file.path(fd_dir, "Sagebrush points.gpkg")
...
f_data <- read_excel(f_data_p, skip = 2, col_types = "guess") %>%
  janitor::clean_names() %>%
  drop_na(block) %>%
  select(block, plot = avenza_plot_number, cover = big_sage) %>%
...
cov_layrs <- cov_layrs[grep("Climate_|MS_|Terrain_|CHM", basename(cov_layrs))]
covs <- rast(cov_layrs)
...
# Build correltation matrix
cov_dat <- spatSample(covs, size = 1e+06, method = "regular") %>% 
  drop_na()
cormat <- cor(cov_dat)

# Determine variables to remove (note: mlr3 uses this function from the caret package under the hood)
covs_corr <- findCorrelation(cormat, names = TRUE, cutoff = 0.9)
covs_corr <- unique(c("CHM", "NDVI", names(cov_dat)[!names(cov_dat) %in% covs_corr]))

# Remove the correlated covariates
covs <- covs[[covs_corr]]
