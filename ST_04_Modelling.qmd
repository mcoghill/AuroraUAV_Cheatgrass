...
ls <- c("tidyverse", "terra", "sf", "mlr3verse", "mlr3spatial", "mlr3benchmark",
...
rr_learners <- rr_filter_results$learner
...
# and queried 
outer_results = as.data.table(
  rr$score(msrs(c("classif.bbrier", "classif.ce", "classif.prauc"))))
...
best_tune <- extract_inner_tuning_results(rr)[best_id, 2:4] 
best_param_set <- best_learner$param_set 
best_varimp <- best_learner$importance()
vip(best_learner, num_features = length(best_varimp))

# Get some model results:
best_agg_score <- rr$aggregate(msr("classif.bbrier"))
ml_prediction <- rr$prediction()
ml_confusion <- ml_prediction$confusion
mod_confusion <- rr$predictions()[[best_id]]$confusion
mod_oob <- best_learner$model$prediction.error
...
# spoof (aggregate) the importance values
cover_rr_learners <- cover_rr_filter_results$learner
...
cover_best_tune <- extract_inner_tuning_results(cover_rr)[cover_best_id, 2:4]
cover_best_param_set <- cover_best_learner$param_set
cover_best_varimp <- cover_best_learner$importance()
vip(cover_best_learner, num_features = length(cover_best_varimp))

# Get some model results:
cover_best_agg_score <- cover_rr$aggregate(msr("regr.rmse"))
cover_ml_prediction <- cover_rr$prediction()
cover_mod_oob <- sqrt(cover_best_learner$model$prediction.error)
...
gr_smote =
  ...
  po("smote", id = "smote1", dup_size = 19) %>>%
  po("smote", id = "smote2", dup_size = 10) %>>%
  po("smote", id = "smote3", dup_size = 2) %>>%
...
plan(multisession, workers = availableCores() %/% 4)
bal_cc_rr_filter <- resample(tsk_bal_cc, bal_cc_lrn_filter, rsmp("loo"), store_models = TRUE)
plan(sequential)

# spoof (aggregate) the importance values
bal_cc_rr_learners <- bal_cc_rr_filter_results$learner
...
# spoof (aggregate) the importance values
unbal_cc_rr_learners <- unbal_cc_rr_filter_results$learner
...
cc_rr_list <- list(bal_cc_rr, unbal_cc_rr, unbal_cc_rr_ng)
...
cc_best_id <- which.min(cc_outer_results[[msr]])
cc_best_learner <- cc_outer_learners[[cc_best_id]]$base_learner()
cc_best_tune <- extract_inner_tuning_results(cc_rr)[cc_best_id, 2:4]
cc_best_param_set <- cc_best_learner$param_set
cc_best_varimp <- cc_best_learner$importance()

# Get some model results:
cc_best_agg_score <- cc_rr$aggregate(msr(msr))
cc_ml_prediction <- cc_rr$prediction()
cc_ml_confusion <- cc_ml_prediction$confusion
mod_confusion <- cc_rr$predictions()[[cc_best_id]]$confusion
mod_oob <- cc_best_learner$model$prediction.error
