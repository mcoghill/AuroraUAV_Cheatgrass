...
rr <- mlr3::resample(
  tsk_pres, at, rsmp("spcv_coords", folds = 8), store_models = TRUE) 
plan(sequential) 
...
# Try this, safer for memory: 
prob_path <- file.path(out_dir, "cheatgrass_presence_prob.tif")
if(file.exists(prob_path)) unlink(prob_path)
prob_predict = predict_spatial(
  covs, best_learner, chunksize = 4096, predict_type = "prob", 
  filename = prob_path)
...
# Place the best resulting learner into a filtering pipeop 
cover_po_filter = po(
  "filter", filter = flt("importance", learner = cover_at_filter$learner), 
...
# Create the tuning design grid - I figured out how to tune across all mtry # values (yay!) 
...
# cover_best_learner$parallel_predict <- FALSE
# plan(sequential)

# Try this, safer for memory: 
cover_path <- file.path(out_dir, "cheatgrass_cover.tif")
if(file.exists(cover_path)) unlink(cover_path)
cover_predict = predict_spatial(
  covs, cover_best_learner, chunksize = 4096, predict_type = "response", 
  filename = cover_path) 
...
#   filename = cover_path, overwrite = TRUE)  
...
cover_classes <- writeRaster(cover_classes, file.path(   
  out_dir, "cheatgrass_cover_class.tif"), overwrite = TRUE)
...
tsk_bal_cc <- as_task_classif(st_drop_geometry(cheat_cc), target = "cover_class")
...
gr_smote =
  ...
  po("smote", id = "smote1", dup_size = 10) %>>%
  po("smote", id = "smote2", dup_size = 10) %>>%
...
# Create the "auto-tuner" for feature filtering (only used to simplify resampling)
bal_cc_at_filter <- auto_tuner(  
  tuner = tnr("grid_search"),  
  learner = bal_cc_lrn_tune,   
  terminator = trm("none"),  
  measure = msr("oob_error"),  
  resampling = rsmp("loo"), # leave one out resampling, should find most accurate feature importance   
  store_models = TRUE 
...
# Train the auto tuned model so that we get the best order of features to be 
# used downstream 
bal_cc_at_filter$train(tsk_bal_cc) 

# Place the best resulting learner into a filtering pipeop 
po_filter = po(
  "filter", filter = flt("importance", learner = bal_cc_at_filter$learner),
  filter.nfeat = to_tune(1, tsk_bal_cc$ncol - 1))  

# Next, create a learner pipeop that will tune the hyperparameters 
bal_cc_lrn_tune = lrn("classif.ranger", importance = "impurity",        
               predict_type = "response",            
               num.trees = to_tune(100, 2000),    
               mtry = to_tune(1, tsk_bal_cc$ncol - 1))  
po_lrn = po("learner", bal_cc_lrn_tune) 

# Create the graph learner object that will combine these 
bal_cc_graph = as_learner(po_filter %>>% po_lrn)  
...
bal_cc_rr <- mlr3::resample(
  tsk_bal_cc, at, rsmp("cv", folds = 8), store_models = TRUE) 
plan(sequential) 

# Get the best model for prediction: first, get all outer learners. These would
# have the most data used for training and testing and will produce the most
# reliable results
bal_cc_data = as.data.table(bal_cc_rr)
bal_cc_outer_learners = bal_cc_data$learner

# From the same object, extract the table of tuning results which can be viewed
# and queried
bal_cc_outer_results = as.data.table(bal_cc_rr$score(msr("oob_error")))

# Now, from the outer learners, extract the one with the lowest binary Brier
# score. Note that if the data is imbalanced, we should probably evaluate 
# models using the area under the precision recall ROC curve (classif.prauc)
# according to the documentation
bal_cc_best_id <- which.min(bal_cc_outer_results$oob_error)
bal_cc_best_learner <- bal_cc_outer_learners[[bal_cc_best_id]]
bal_cc_best_tune <- bal_cc_data$learner[[bal_cc_best_id]]$tuning_result[, 1:3]
bal_cc_best_param_set <- bal_cc_best_learner$learner$param_set
...
# using the best learner, generate a map prediction from the predictor rasters
# Set the num.threads value to use all cores
bal_cc_best_learner$learner$param_set$values$classif.ranger.num.threads <- availableCores()


cc_path <- file.path(out_dir, "cheatgrass_cover_class.tif")
if(file.exists(cc_path)) unlink(cc_path)
bal_cc_predict = predict_spatial(
  covs, bal_cc_best_learner$learner, predict_type = "response", chunksize = 4096,
  filename = cc_path)
